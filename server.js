...
let players = [];
let gameState = {
  paddle1: { x: 0.5, y: 0.5667, score: 0, vx: 0, vy: 0 }, // Изменено
...
function updateGame() {
  ...
  if (gameState.ball.dx === 0 && gameState.ball.dy === 0) {
    ...
    gameState.ball.y = gameState.servingPlayer === 1 ? 0.6 : 0.9; // Изменено
  } else {
  ...
  if (collision1.hit) {
    ...
    if (
      ...
      gameState.ball.dy = -speed; // Изменено направление
      const normalized = normalizeSpeed(
        gameState.ball.dx,
        gameState.ball.dy,
        speed
      ...
      gameState.ball.dx = normalized.dx;
      gameState.ball.dy = normalized.dy;
      gameState.ball.y = gameState.paddle1.y - ballRadius; // Изменено
      ...
      gameState.ball.dx = hitPos * 0.004 + gameState.paddle1.vx * 0.5;
      gameState.ball.dy = -Math.abs(gameState.ball.dy) * 0.8 - baseSpeed; // Изменено
      const normalized = normalizeSpeed(
        gameState.ball.dx,
        gameState.ball.dy,
        speed
...
function startGame() {
  ...
  gameState.paddle1 = { x: 0.5, y: 0.5667, score: 0, vx: 0, vy: 0 }; // Изменено
  ...
  ws.on("message", (message) => {
    try {
      ...
      if (
        ...
        paddle.y = constrain(
          position.y,
          data.playerId === 1 ? 0.5 : 0.6, // Изменено
          data.playerId === 1 ? 0.6 : 1 - 0.0333
        );
